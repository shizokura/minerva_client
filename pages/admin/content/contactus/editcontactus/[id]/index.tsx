...
const EditContactUsPage: FC = () => {
  ...
  const [ contactUsD, setContactUsD ] = useState<[]>()

  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`https://minervasales-23b0919d60c1.herokuapp.com/contact/getAllAContact/${router.query.id}`, {
        method: "GET",
  ...
  const EditAboutUsForm = async (e: any) => {
    ...
    // fd.append('userID', userId);
      ...
      const response = await fetch(`https://minervasales-23b0919d60c1.herokuapp.com/contact/updateContact/${router.query.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  useEffect(() => {
    contactUsD?.map(({ userID, aboutID, id, title, description,}: any) => {
        setContactUs({
          title: title,
    ...
    <div className="flex w-full h-[1050px] bg-gradient-to-r from-amber-200 to-yellow-500 flex-col bg-white bg-clip-border text-gray-700 shadow-md">
            <div className="pt-10 md:pl-96 md:pt-[46px] lt:pl-20 lg:pl-96">
                    <div className="p-4 md:p-8">
                        <h1 className="text-black text-center font-bold pb-8 text-4xl md:text-5xl lg:text-6xl">Edit Contact Us Details</h1>
                                    ...
                                    <div className="md:w-4/5 lg:w-3/4 xl:w-2/3">
                                            <div className="flex flex-col md:flex-row">
                                                ...
                                                <input id="title" type="text" name="title"
                                                    ...
                                                    onChange={(e) => setContactUs({...contactUs, title: e.target.value})}
                                                    value={contactUs.title} required/>
                                            ...
                                            <div className="flex flex-col md:flex-row">
                                                ...
                                                onChange={(e) => setContactUs({...contactUs, description: e.target.value})}
