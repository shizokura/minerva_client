...
const ServicePage = () => {
  ...
  const [ serviceSearch, setServiceSearch ] = useState<any>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
       const response = await fetch(`https://minervasales-23b0919d60c1.herokuapp.com/services/getAllServices/?skip=${page}&orderby=asc`, {
          method: "GET",
    ...
    useEffect(() => {
      const fetchData = async () => {
        const response = await fetch(`https://minervasales-23b0919d60c1.herokuapp.com/services/getSearchServices/?search=${search}`, {
          method: "GET",
          headers: { 'Content-Type': 'application/json' },
        })
        const result = await response.json()
        setServiceSearch(result)
      }
      fetchData()
        ...
        <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div className="inline-block min-w-full shadow rounded-lg overflow-hidden h-[790px] bg-white">
                <table className="min-w-full leading-normal">
                    ...
                    <tbody>
                    {search ? serviceSearch?.map(({ userId, servicesID, id, services, description, price, status, }: any) => (
